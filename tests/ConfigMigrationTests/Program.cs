...
// The 2.0.1 defaults, exactly as ModConfig.cs binds them.
var NewDefaults = new (string Section, string Key, object Value)[]
{
    ...
    // v1 rows, still declared because a v0 file passes through them on its way to v2.
    ("2. Crude Wings",  "MinStationLevel", 3),
    ("2. Crude Wings",  "CraftingRequirements", "BronzeNails:10,DeerHide:10,LeatherScraps:20,TrollHide:5,Feathers:20"),
    ("3. Troll Wings",  "MinStationLevel", 3),
...
Console.WriteLine("--- a stock 2.0.0 file (ConfigVersion 1) upgrading to 2.0.1 ---");
var (stock, stockVersion) = Migrate(TESTER, "stock.cfg");
Check("config version is stamped 2", stockVersion.Value == 2, stockVersion.Value.ToString());
...
Check("the log says which values were kept",
      Jotunn.Logger.Lines.Any(l => l.Contains("was customised") && l.Contains("4000")));

Console.WriteLine();
Console.WriteLine("--- a 1.1.x file (unstamped, version 0) jumping straight to 2.0.1 ---");
string v0 = Path.Combine(FIXTURES, "v0-unstamped-1.1.x.cfg");
var (old, oldVersion) = Migrate(v0, "v0.cfg");
Check("config version is stamped 2", oldVersion.Value == 2, oldVersion.Value.ToString());
...
Check("the v1 station-level rebase still applies", (int)ValueOf(old, "2. Crude Wings", "MinStationLevel") == 3);
...
// Re-stamp the file as version 2 the way Finish + BepInEx would have written it back.
var lines = File.ReadAllLines(Path.Combine(WORK, "again-source.cfg"))
    .Select(l => l.StartsWith("ConfigVersion = ") ? "ConfigVersion = 2" : l).ToList();
...
Check("an already-migrated file is left alone", !Jotunn.Logger.Lines.Any(l => l.Contains("Migrating")));
Check("no second backup is written", !File.Exists(Path.Combine(WORK, "again.cfg.v2.bak")));
Check("version stays 2", againVersion.Value == 2);
...
Check("a fresh install stamps the current version and logs no migration",
      freshVersion.Value == 2 && !Jotunn.Logger.Lines.Any(l => l.Contains("Migrating")));
